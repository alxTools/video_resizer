...
def convert_video(input_file, output_file, resolution, crf_value, audio_option, sample_rate, bit_depth):
    ...
    if audio_option == "Copy Audio":
        ...
        command.extend([
            '-c:a', 'aac',
            '-b:a', '512k',  # High bitrate for fixed bitrate encoding
            '-ar', f'{sample_rate}',  # Set audio sample rate
        ])

    # Add resolution-specific options
    if resolution == "4K":
        command.extend([
            '-vf', 'scale=3840:-1',  # Explicitly define the scaling
            '-c:v', 'hevc_nvenc',
            '-preset', 'p7',
            '-profile:v', 'main',  # Switch to main profile for more compatibility
        ...
        command.extend([
            '-vf', 'scale=7680:-1',  # Explicitly define the scaling
            '-c:v', 'hevc_nvenc',
            '-preset', 'p7',
            '-profile:v', 'main',  # Switch to main profile for more compatibility
            '-rc', 'vbr',  # Use high-quality VBR mode
